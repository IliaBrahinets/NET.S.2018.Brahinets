...
namespace IListExtensions
{
    public static class MergeSortExtension
    {
        ...
        public static void MergeSort<T>(this IList<T> arr, int? left, int? right, IComparer<T> comparer)
        {
            ...
            _MergeSort(arr, _left, _right, comparer);
        }

        private static void _MergeSort<T>(IList<T> arr, int left, int right, IComparer<T> comparer)
        {
            if (left == right)
            {
                return;
            ...
            _MergeSort(arr, left, middle, comparer);
            _MergeSort(arr, middle + 1, right, comparer);
